...
WEAK void* glassHeapAlloc(size_t size) { return kygxAlloc(GX_MEM_HEAP, size); }
...
WEAK void* glassLinearAlloc(size_t size) { return kygxAlloc(GX_MEM_LINEAR, size); }
WEAK void glassLinearFree(void* p) { kygxFree(p); }
WEAK size_t glassLinearSize(const void* p) {kygxGetAllocSize(p); }
WEAK bool glassIsLinear(const void* p) { kygxIsLinear(p); }

WEAK void* glassVRAMAlloc(GXVRAMBank bank, size_t size) { return kygxAllocVRAM(bank, size); }
